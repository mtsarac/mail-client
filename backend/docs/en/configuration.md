...
environment variable. Copy `.env.example` to `.env` (never commit it); `.env` is
read by Docker Compose only — `dotnet run` does not load it (see
...
| `Proxy__KnownProxies__N`, `Proxy__KnownNetworks__N` | Trusted reverse proxies (enables `X-Forwarded-*`); also decides the client IP used for anonymous rate limiting | none |
...
| HSTS | no | yes |
| Kestrel `Server` header | suppressed | suppressed |
| Error `detail` | exception text | hidden |

Malformed JSON request bodies return `400` with code `invalid_request` in every
environment.
